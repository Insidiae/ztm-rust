...
pub async fn get_clip(
	...
	match action::get_clip(shortcode.clone().into(), database.get_pool()).await {
		...
		Err(e) => match e {
			...
			ServiceError::NotFound => Err(PageError::NotFound(String::from("Clip not found"))),
			_ => Err(PageError::Internal(String::from("Server error"))),
		},
	}
}
...
pub mod test {
	...
	fn requires_password_when_applicable() {
		...
		//? Block raw clip when no password is provided
		let response = client
		...
		//? Get raw clip when the password is provided
		let response = client
			.get(format!("/clip/raw/{}", clip.shortcode.as_str()))
		...
		//? Get raw clip when the password is provided, but incorrect
		let response = client
			.get(format!("/clip/raw/{}", clip.shortcode.as_str()))
