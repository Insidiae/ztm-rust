...
#[derive(Debug, Responder, Serialize, thiserror::Error)]
...
impl From<ServiceError> for ApiError {
	fn from(err: ServiceError) -> Self {
		match err {
			...
			ServiceError::NotFound => Self::NotFound(Json(String::from("Entity not found"))),
			ServiceError::Data(_) => Self::Server(Json(String::from("A server error has occured"))),
...
impl<'r> FromRequest<'r> for ApiKey {
	...
	async fn from_request(req: &'r Request<'_>) -> Outcome<Self, Self::Error> {
		...
		match req.headers().get_one(API_KEY_HEADER) {
			None => key_error(ApiKeyError::NotFound(String::from("API Key not found"))),
			Some(key) => {
				...
				match action::api_key_is_valid(api_key.clone(), db.get_pool()).await {
					...
					Ok(valid) if !valid => {
						key_error(ApiKeyError::NotFound(String::from("API Key not found")))
					}
					_ => server_error(),
				}
			}
...
pub async fn get_clip(
	...
	let req = service::ask::GetClip {
		...
		password: cookies
			...
			.map(|raw_password| Password::new(String::from(raw_password)).ok())
			.flatten()
